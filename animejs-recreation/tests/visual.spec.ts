...
test.describe('layout invariants', () => {
  ...
  test('lens overlay is centred on the projected display', async ({ page }) => {
    ...
    expect(box, 'an active lens slot exists').not.toBeNull();
    // The feature sections put the lens dead centre.
    expect(Math.abs(box!.cx - REFERENCE.width / 2)).toBeLessThan(24);
    expect(Math.abs(box!.cy - REFERENCE.height / 2)).toBeLessThan(24);
